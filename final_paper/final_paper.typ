...
Notice that as the radius $r$ increases, more and more edges are connected, leading to emergence and disappearance of topological features. We thus can characterize these topological features by their emergence time and disappearance time, or birth time and death time using standard topology terminology. For instance, recall our previous example concerning points $x_1$ and $x_2$. If a 1-dimensional hole $alpha$ emerges because of the addition of the edge between them at $r_1$, then we would say that $alpha$ has a birth time of $r_1$. Similarly, if at some later radius $r_2 > r_1$ the hole $alpha$ disappears because of the addition of another edge, then we would denote $r_2$ as the death time of $alpha$.
...
Now given two sets of points, we can compute separately their persistence diagrams using persistent homology, but we need a metric to compare the two persistence diagrams. _Wasserstein distance_ is the most common metric for this task. Given persistence diagrams $D_1$ and $D_2$, Wasserstein distance finds the best matching between points in $D_1$ and $D_2$ and computes the sum of distances between matched points. Formally, let $p$ be a fixed dimension. The $p$-Wasserstein distance between $D_1$ and $D_2$ is defined as:
...
This section presents the methodology used in this study. @sec:model gives an overview of the model selected in the study. Then, @sec:datasets introduces the datasets from which the sentences for analysis are drawn. This section closes with @sec:experiment_design that details the experimental design and analysis procedure.
...
== Experiment Design <sec:experiment_design>
...
@fig:bleu_distributions below shows the distributions of BLEU scores for translations in the French-English and Chinese-English datasets. From the plots, we note that the Chinese-English language pair achieves lower BLEU scores on average than French-English by approximately 10 points. In particular, perfect translation ($"BLEU" = 100$) is not uncommon between French and English but rarely seen in Chinese-English translations. This phenomenon is expected because French and English come from the same typological roots, while Chinese belongs to a completely different language family. Therefore, it is generally more difficult to translate between Chinese and English than between French and English. A more specific analysis of translation errors is presented in @sec:model_limitations.
